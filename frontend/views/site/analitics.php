...
{
    $color = array('#FF0F00', '#FF6600', '#FF9E01', '#FCD202', '#F8FF01', '#B0DE09', '#04D215', '#0D8ECF', '#0D52D1', '#2A0CD0', '#8A0CCF', '#CD0D74', '#FF0F00', '#FF6600', '#FF9E01', '#FCD202', '#F8FF01', '#B0DE09', '#04D215', '#0D8ECF', '#0D52D1', '#2A0CD0', '#8A0CCF', '#CD0D74');
...
foreach ($arr as $res) {
    $i++;
    $DATA_GRAF[] = ['color' => random_html_color($i), 'city_name' => $BRANCH_KEYS[$res['id_filial']]['name'], 'city_count' => $res['count']];
}
...
<div class="row">
    ...
    <div class="col-md-6 bg-light">
        ...
        echo Chart::widget(['type' => Chart::TYPE_DOUGHNUT, 'labels' => $arrUser, 'datasets' => [

            ['label' => 'По Сотрудникам', 'query' => \app\models\Deals::find()->select('id_operator')->addSelect('count(*) as data')->groupBy('id_operator')->createCommand(), 'labelAttribute' => 'id_operator',
    ...
    <div class="col-md-12">
        <div class="card card-info">
            <div class="card-body">
                ...
                //            ActiveForm::end();
                ?>
                <?= Chart::widget(['type' => Chart::TYPE_BAR, 'labels' => $arrLabel, 'datasets' => [


                    ['label' => 'По филиалам', 'query' => \app\models\Deals::find()->select('id_filial')->addSelect('count(*) as data')->groupBy('id_filial')->createCommand(), 'labelAttribute' => 'id_filial',
